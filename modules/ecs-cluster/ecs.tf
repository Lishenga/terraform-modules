...
resource "aws_launch_configuration" "cluster" {
  ...
  image_id             = var.image_id != null ? var.image_id : data.aws_ami.ecs.id
  /* image_id             = "ami-093cfc6e8a3d255f9" */
  instance_type        = var.instance_type
  key_name             = var.ssh_key_name
  iam_instance_profile = aws_iam_instance_profile.cluster-ec2-role.id
  security_groups      = [aws_security_group.cluster.id]
  user_data            = templatefile("${path.module}/templates/ecs_init.tpl", { cluster_name = var.cluster_name })
